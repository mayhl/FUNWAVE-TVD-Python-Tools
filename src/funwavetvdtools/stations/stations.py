...
class RealStationData(Station, StationData):
    ...
    def _load_data(self):
        ...
        fpath = RealStationData.generate_file_path(dpath, self._num)
        fv.check_fpath(fpath, 'station')
        ...
        # Set all variable loaded flags to true and performing a safety check
        for name in self._var_names:
            var = self._vars[name]
            var.is_loaded = True 
            if var.data is None:
                desc = "FATAL: Station variable '%s' data not set correctly! Please report error." % name
                raise FunException(desc, NameError) 
...
class Stations(Sequence, Iterator):
    ...
    def from_file(cls, fpath, dpath, x, y, nums=None, bathy=None):
        ...
        if not bathy is None: stas_obs.get_depths(bathy)
        return  stas_obj
